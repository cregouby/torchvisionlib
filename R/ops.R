...
#'
#' @details
#' The RoI Align operator pools features from a feature map by using bilinear interpolation
#' to sample the input at non-integer locations. This avoids the quantization effects of
#' RoI Pool and provides better alignment between the RoI and the extracted features.
#'
#' The `aligned` parameter controls the pixel coordinate system:
#' * `aligned = FALSE` (default): Uses the legacy implementation where a continuous
#'   coordinate p is discretized to `floor(p)`. This maintains backward compatibility.
#' * `aligned = TRUE`: Uses the Detectron2 implementation where pixels are shifted by
#'   -0.5 for better alignment. This provides improved accuracy and is recommended for
#'   new applications.
#'
#' @param input (`Tensor[N, C, H, W]`): The input tensor, i.e. a batch with `N` elements. Each element
#'   contains `C` feature maps of dimensions `H x W`.
...
#'   <= 0, then an adaptive number of grid points are used (computed as
#'   `ceil(roi_width / output_width)`, and likewise for height). Default: -1
#' @param aligned (bool): If `FALSE` (default), use the legacy implementation.
#'   If `TRUE`, pixel shift the box coordinates by -0.5 for better alignment. See Details.
#'
#' @returns
#'   `Tensor[K, C, output_size[1], output_size[2]]`: The pooled RoIs. The output preserves
#'   all input channels (unlike position-sensitive variants).
#'
#' @examples
#' if (torchvisionlib_is_installed()) {
#' library(torch)
...
#' print(output$shape)  # [1, 3, 2, 2]
#'
#' # Using the module interface
#' roi <- nn_roi_align(output_size = c(2, 2))
#' output <- roi(input, boxes)
#' print(output$shape)  # [1, 3, 2, 2]
#' }
#'
#' @export
ops_roi_align <- function(input, boxes, output_size, spatial_scale = 1.0,
  ...
  rois[,1] <- rois[,1] - 1
  rcpp_vision_ops_roi_align(input, rois, spatial_scale, output_size[1],
                            output_size[2], sampling_ratio, aligned)
}


#' @describeIn ops_roi_align The [torch::nn_module()] wrapper for [ops_roi_align()].
#' @export
nn_roi_align <- torch::nn_module(
  initialize = function(output_size, spatial_scale = 1.0, sampling_ratio = -1,
                        aligned = FALSE) {
    self$output_size <- output_size
    self$spatial_scale <- spatial_scale
    self$sampling_ratio <- sampling_ratio
  ...
  forward = function(input, rois) {
    ops_roi_align(input, rois, self$output_size, self$spatial_scale,
