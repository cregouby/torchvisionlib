...
#'   library(torch)
#'   value <- torch_randn(1, 100, 256)
#'   spatial_shapes <- torch_tensor(matrix(c(10,10, 5,5), ncol=2, byrow=TRUE), dtype = torch_long())
#'   level_start_index <- torch_tensor(c(0, 100), dtype = torch_long())
#'   sampling_loc <- torch_rand(1, 10, 2, 4, 2)
#'   attn_weight <- torch_rand(1, 10, 2, 4) %>% nnf_softmax(dim=3)
#'
